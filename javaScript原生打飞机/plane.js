;window.onload = function () {
    ...
    var bulletW = 6,
        bulletH = 14;
    //声明需要使用到的全局变量
    var gameStartus = false,//当前的游戏状态
        a = null,//创建子弹的定时器
        b = null,//创建敌机的定时器
        c = null,//背景图运动的定时器
        backgroundPY = 0,//背景图y轴上的值
        bullets = [],//所有子弹元素的集合
        enemys = [],//所有敌机元素的集合
        scores = 0;//得分
    ...
    function danger(enemy) {
        for (var i = 0;i < bullets.length;i ++){
            ...
            var condition = bulletL + bulletW >= enemyL && bulletL <= enemyL + enemyW &&
                bulletT <= enemyT + enemyH && bulletT + bulletH >= enemyT;
            if(condition){
                //子弹和敌机的碰撞，删除子弹
                //先清除碰撞子弹的定时器
                ...
                if(enemy.hp == 0){
                    ...
                    //替换爆炸图片
                    enemy.src = 'images/bz' + enemy.t + '.gif';
                    //标记死亡敌机
                    enemy.dead = true;
                    //计算得分
                    scores +=enemy.score;
        ...
        for(var i = 0; i < enemys.length;i ++){
            if(!enemys[i].dead){//游戏界面内存活的敌机
                ...
                if (condition){//己方飞机和敌机碰撞
                    ...
                    myPlane.style.top = gameH - myPlaneH + 'px';//为什么不能直接看buttom
                }
            }
        }
    }
